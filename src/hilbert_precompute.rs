...
use crate::{bit_count, interleave_bits, mask, mask_pow2_and_under, Interleavable};

/// HilbertPrecompute
#[derive(PartialEq, Eq, Hash, Clone, Copy, Debug)]
pub struct HilbertPrecomputeData<T>
where
    T: Interleavable,
{
    flip: T,
...
impl<T> HilbertPrecomputeData<T>
where
    T: Interleavable,
{
...
/// HilbertPrecompute
#[derive(PartialEq, Eq, Hash, Clone, Copy, Debug)]
pub struct HilbertPrecompute<T>
where
    T: Interleavable,
{
    x: T,
...
impl<T> PartialEq<(T, T)> for HilbertPrecompute<T>
where
    T: Interleavable,
{
    #[inline]
...
impl<T> PartialOrd<(T, T)> for HilbertPrecompute<T>
where
    T: Interleavable,
{
    #[inline]
...
impl<T> PartialOrd for HilbertPrecompute<T>
where
    T: Interleavable,
{
    #[inline]
...
impl<T> Ord for HilbertPrecompute<T>
where
    T: Interleavable,
{
    #[inline]
...
fn xy_local_dist<T>(log2_n: usize, x: T, y: T, flip: T, swap: T) -> T::Wider
where
    T: Interleavable,
{
    let bits_wide = bit_count::<T>();
